...
for ((year=9; year<=input_endyear; year++)); do
		...
		rm $path/20${year2}_${month}.*
		cp $path/clean_data/20${year2}_${month}.csv $path/.
		...
		cut -d " " -f 1- $path/20${year2}_${month}.csv | sponge $path/20${year2}_${month}.csv
		cat $path/20${year2}_${month}.csv | tr "|" "," | sponge $path/20${year2}_${month}.csv
		head -n 500 $path/20${year2}_${month}.csv | cut -d "," -f 2- | sponge $path/20${year2}_${month}.csv
		gawk 'BEGIN {FS = ",";OFS = "," }; {print $1,$2,$3}' $path/20${year2}_${month}.csv | sponge $path/20${year2}_${month}.csv
		...
		gawk 'BEGIN {FS = ",";OFS = "," }; NR > 1 {print $0,$3*0.5/33.75}' $path/20${year2}_${month}.csv > $path/20${year2}_${month}_size.csv
		...
		#rm $path/20${year2}_${month}_size.csv
